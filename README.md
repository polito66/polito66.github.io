## Curso sobre Marcado de texto digital

### 01 Introducción al Marcado

- [Introducción de conceptos](teach-xml-es/01-Intro-Markup-ES.pptx)

### 02/03 Trabajo práctico

- [Manual-Introducción a Oxygen y a editar en XML](teach-xml-es/02-IntroOxygen-Markup-Practical.pdf)

### 04 Caso de estudio Diario

- [Manual-Editar el diario](teach-xml-es/04-Colon-Markup.pdf)
- Texto del diario de Colón en formato .txt
[e019-excerpt.txt](teach-xml-es/e019-excerpt.txt)

### 05 Visualizar y Buscar en XML

- [Manual-Visualizar y buscar](teach-xml-es/05-XML-Next.pdf)
- [Cookbook XML básico](teach-xml-es/cookbook-plain.xml)
- [Cookbook XML con enlace CSS](teach-xml-es/cookbook-css.xml)
- [Prueba CSS 1](teach-xml-es/test.css)
- [Prueba CSS 1](teach-xml-es/test2.css)

### Otros materiales
- [Extra - La entretenida](teach-xml-es/Extra-Entretenida.pdf)
